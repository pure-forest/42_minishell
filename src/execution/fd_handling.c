...
/*   Updated: 2025/04/26 17:30:08 by gboggion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_std_fds(t_struct_ptrs *data, t_input *input, t_exec_data *exec_data)
{
	if (input->input_fd != -1)
	{
		if (dup2(input->input_fd, STDIN_FILENO) == -1)
			return (handle_fd_err(data, exec_data, "Dup2 Failure", YES), FAIL);
	}
	...
	if (input->output_fd >= 0)
	{
		if (input->output_fd != STDOUT_FILENO)
			if (dup2(input->output_fd, STDOUT_FILENO) == -1)
				return (handle_fd_err(data, exec_data, "Dup2 Fail", YES), FAIL);
	}
...
int	handle_standard_fds(t_struct_ptrs *data, t_exec_data *exec_data, int reset)
{
	...
	if (reset == NO)
	{
		...
		if (exec_data->orig_stdin == -1)
			return (handle_fd_err(data, exec_data, "Dup Fail", YES), SYS_FAIL);
	...
	if (reset == YES)
	{
		...
		if (res == -1)
			return (handle_fd_err(data, exec_data, "Dup2 Fail", YES), SYS_FAIL);
