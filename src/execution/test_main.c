...
int	main(int ac, char **av, char **envp)
{
	...
	if (data.input)
	{
		...
		if (data.input->redir_in)
			free(data.input->redir_in);
		if (data.input->redir_out)
			free(data.input->redir_out);
		free(data.input);
	}
	return (0);
}
